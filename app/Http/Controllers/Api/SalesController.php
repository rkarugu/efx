...
*/
    ...
    public function getUserShops(Request $request): JsonResponse
    {
        try {
            $user = JWTAuth::toUser($request->token);
            if (!$user) {
                return response()->json(['message' => 'The provided token does not match any user'], 422);
            }
            ...
            if (!$currentShift) {
                ...
                $shops = WaRouteCustomer::with(['center'])->where('route_id', $currentShift->route_id);
                $route = Route::find($currentShift->route_id);
                ...
                $shops = $shops->orderBy('created_at', 'DESC')
                    ...
                    ->through(function (WaRouteCustomer $shop) use ($appUrl, $request, $user, $route) {
                        ...
                        $shop->is_centre_active = $shop->center?->is_active;
                        switch ($user->role_id) {
                            case 4:
